 ...
 */
public abstract class AbstractIInteractiveElementAssert<S extends AbstractIInteractiveElementAssert<S, A>, A extends IInteractiveElement> extends
    AbstractObjectAssert<S, A> {

  /**
   * Creates a new <code>{@link AbstractIInteractiveElementAssert}</code> to make assertions on
   * actual ArmaElement.
   *
   * @param actual the ArmaElement we want to make assertions on.
   */
  protected AbstractIInteractiveElementAssert(A actual, Class<S> selfType) {
  ...
  public S containsClass(String expectedClass) {
    ...
    if (!StringUtils.contains(actual.getAttribute("class"), expectedClass)) {
      failWithMessage("\nExpecting that element contains [%s] class but it does not.",
          expectedClass);
    }
  ...
  public S notContainsClass(String expectedClass) {
    ...
    // check that property call/field access is true
    if (StringUtils.contains(actual.getAttribute("class"), expectedClass)) {
      failWithMessage("\nExpecting that element does not contain [%s] class but it does.",
          expectedClass);
    }
  ...
  public CharSequenceAssert value() {
    ...
    charSequenceAssert.as("Value of element " + buildDescription());
  ...
  public CharSequenceAssert valueIgnoringNewLines() {
    ...
    charSequenceAssert.as("Value of element " + buildDescription() + " ignoring new lines");
    return charSequenceAssert;
  }
  ...
  public CharSequenceAssert attribute(String attribute) {
    Preconditions.checkArgument(StringUtils.isNotBlank(attribute),
        "name of an attribute cannot be null or blank string");
  ...
  private String buildDescription() {
    if (actual instanceof INamed) {
      return ((INamed) actual).getLoggableName() + " => " + actual.getLocatorsPath();
    } else {
      return null;
    }
  }
